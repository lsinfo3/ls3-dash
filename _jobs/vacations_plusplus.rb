...
def days_of_vacation_next_week_for_count(vacation_events, name, type, start_week, end_week)
  vacation_events.select do |e|
    e.summary.to_s == name
  end.select do |e|
    !(start_week > e.dtend.to_date || end_week < e.dtstart.to_date) && (e.categories.first== type)
...
SCHEDULER.every '1h', first_in: 0 do
  ...
  today = Date.today
  tomorrow = today + 1
  this_monday = today.next_day(-(today.wday - 1))
  this_friday = this_monday.next_day(4)
  ...
  vacations_this_week.each do |entry|
    entry[:count] = days_of_vacation_next_week_for(vacations.events, entry[:label], entry[:type], this_monday, this_friday)[2..-1]
    entry[:count] << " (" << days_of_vacation_next_week_for_count(vacations.events, entry[:label], entry[:type], this_monday, this_friday).to_s << ")"
  end
  ...
  vacations_next_week.each do |entry|
    ...
    entry[:count] << " (" << days_of_vacation_next_week_for_count(vacations.events, entry[:label], entry[:type], next_monday, next_friday).to_s << ")"
  end
